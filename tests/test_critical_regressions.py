...
os.environ.setdefault("ALIPAY_APP_ID", "test-alipay-app-id")
...
def test_dataset_save_route_requires_current_user_dependency():
    signature = inspect.signature(dataset_router.save_annotated_dataset)

    dependency = signature.parameters["current_user"].default.dependency
    assert dependency is dataset_router.get_current_user
...
def test_payment_callback_rejects_mismatched_app_id(monkeypatch):
    class ExplodingDb:
        def exec(self, *args, **kwargs):
            raise AssertionError("mismatched callbacks must not query or update local orders")

    monkeypatch.setattr(payment_service, "APP_ID", "expected-app-id")
    result = payment_service.process_callback(
        db=ExplodingDb(),
        data={
            "sign": "valid-signature",
            "app_id": "attacker-app-id",
...
def test_payment_callback_rejects_mismatched_seller_id(monkeypatch):
    class ExplodingDb:
        def exec(self, *args, **kwargs):
            raise AssertionError("mismatched callbacks must not query or update local orders")

    monkeypatch.setattr(payment_service, "APP_ID", "expected-app-id")
    monkeypatch.setattr(payment_service, "SELLER_ID", "expected-seller-id")
    result = payment_service.process_callback(
        db=ExplodingDb(),
        data={
            "sign": "valid-signature",
            "app_id": "expected-app-id",
            "seller_id": "attacker-seller-id",
            "trade_status": "TRADE_SUCCESS",
            "out_trade_no": "ORDER_1",
            "trade_no": "ALI_1",
            "total_amount": "9.90",
